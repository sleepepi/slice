...
```
SHELL=/bin/bash
0 1 * * * source /etc/profile.d/rvm.sh && cd /var/www/slice && /usr/local/rvm/gems/ruby-2.3.2/bin/bundle exec rake daily_digest RAILS_ENV=production
0 1 * * * source /etc/profile.d/rvm.sh && cd /var/www/slice && /usr/local/rvm/gems/ruby-2.3.2/bin/bundle exec rake passwords:expire RAILS_ENV=production
0 2 * * * source /etc/profile.d/rvm.sh && cd /var/www/slice && /usr/local/rvm/gems/ruby-2.3.2/bin/bundle exec rake sitemap:refresh RAILS_ENV=production
```

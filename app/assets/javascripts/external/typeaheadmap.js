...
!function($){
  ...
  * ================================= */
  ...
  , matcher: function (item) {
      return ~item[this.value].toLowerCase().indexOf(this.query.toLowerCase())
    }
  ...
  , highlighter: function (item, that) {
      ...
      return that.displayer(that, item, item[that.value].replace(new RegExp('(' + query + ')', 'ig'), function ($1, match) {

    ...
    if @design
      ...
      if @column_variable
        ...
        column_strata.each do |stratum|
          ...
          @ranges << { name: (((stratum[:value].blank? or stratum[:value] == stratum[:name]) ? '' : stratum[:value] + ": ") + stratum[:name]).truncate(10), tooltip: stratum[:name], start_date: '', end_date: '', scope: scope, count: scope.count }
        end

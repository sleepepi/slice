class SubjectSchedule < ActiveRecord::Base
  # Named Scopes
  scope :with_subject_status, -> (arg) { includes(:subject).where(subjects: { status: arg, deleted: false }) }

  # Model Validation
  validates :subject_id, :schedule_id, presence: true
  ...
  def destroy
    sheets.each { |s| s.update(event_id: nil, subject_schedule_id: nil, subject_event_id: nil) }
    super
  end

  def offset_date(interval, units)
    return unless initial_due_date

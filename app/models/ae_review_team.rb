...
class AeReviewTeam < ApplicationRecord
  ...
  has_many :principal_reviewers, -> { current.where(ae_review_team_members: { principal_reviewer: true }) }, through: :ae_review_team_members, source: :user
  ...
  has_many :no_role_users, -> { current.where(ae_review_team_members: { manager: false, principal_reviewer: false, reviewer: false, viewer: false }) }, through: :ae_review_team_members, source: :user

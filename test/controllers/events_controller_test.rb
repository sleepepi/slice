...
class EventsControllerTest < ActionController::TestCase
  ...
  test 'should create event with two designs' do
    assert_difference('Event.count') do
      post :create, project_id: @project, event: { name: 'New Event', description: @event.description, design_hashes: [{ design_id: designs(:one).id, handoff_enabled: '0' }, { design_id: designs(:all_variable_types).id, handoff_enabled: '1' }] }
    end

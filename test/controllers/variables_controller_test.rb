...
class VariablesControllerTest < ActionController::TestCase
  ...
  test 'should add grid row' do
    post :add_grid_row, id: variables(:grid), project_id: @project, design_option_id: design_options(:has_grid_grid), format: 'js'
    assert_not_nil assigns(:variable)
  ...
  test 'should add grid row using authentication token' do
    post :add_grid_row, id: variables(:external_grid), project_id: sheets(:external).project, design_option_id: design_options(:admin_public_design_external_grid), sheet_authentication_token: sheets(:external).authentication_token, format: 'js'
    assert_not_nil assigns(:variable)
    assert_template 'add_grid_row'
    assert_response :success
  end

...
class ApplicationHelperTest < ActionView::TestCase
  ...
  test "should show recent activity" do
    ...
    assert recent_activity(Time.now).is_a?(String)
    assert recent_activity(Time.now - 12.hours).is_a?(String)
    assert recent_activity(Time.now - 1.day).is_a?(String)
    assert recent_activity(Time.now - 2.days).is_a?(String)
    assert recent_activity(Time.now - 1.week).is_a?(String)
    assert recent_activity(Time.now - 1.month).is_a?(String)
    assert recent_activity(Time.now - 6.month).is_a?(String)
    assert recent_activity(Time.now - 1.year).is_a?(String)
    assert recent_activity(Time.now - 2.year).is_a?(String)
  end

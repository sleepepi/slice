...
namespace :sheets do
  ...
  desc "Set initial language code for sheets"
  task set_language_code: :environment do
    print "Setting initial language code for sheets..."
    SheetTransaction.where(language_code: nil).update_all(language_code: World.default_language)
    Sheet.where(initial_language_code: nil).find_each do |sheet|
      sheet.update initial_language_code: sheet.sheet_transactions.reorder(:id).first&.language_code
    end
    puts "DONE"
  end
end
